...
### 1.2 Inputs
# SV
all_dsv <- read.table("01.cleanData/SV_all/France_dsgv_all.tsv",check.names = F)
all_vsv <- read.table("01.cleanData/SV_all/France_vsgv_all.tsv",check.names = F)
...
tiff(file = "pics/FS2_France_read_count_sv_number.tiff", width =1000, height =800, res =300) 
print(p_scatter)
dev.off()
...
tiff(file = "pics/FS1_France_Informative_species_number.tiff", width =4000, height =3200, res =300) 
print(p_species_n)
dev.off()
...
tiff(file = "pics/F2_France_Total_SVs_number.tiff", width =2300, height =2300, res =300) 
print(p_pie)
dev.off()

tiff(file = "pics/F2_France_Total_SVs_number_nolabel.tiff", width =2300, height =2300, res =300) 
print(p_pie_nolabel)
dev.off()
...
p_sv_species<-ggplot(species_sgv_n_long, aes(x=Short_name, y=number, group = type, fill = type))+
  ...
  scale_fill_manual(name=NULL,
                      ...
                      values = mycolor2_blue_yellow)+ggtitle("France")+
  theme(plot.subtitle = element_text(vjust =1), 
        plot.caption = element_text(vjust = 1), 
...
tiff(file = "pics/F2_France_SVs_number_species.tiff", width =5000, height =2200, res =300) 
print(p_sv_species)
dev.off()
...
p_sample_n<-ggplot(infor_sample_n.long, aes(x=Short_name, y=Sample_size,group = Cohort))+
  ...
  scale_y_continuous(expand = c(0,0))+
  scale_fill_manual(name=NULL,
                      breaks = c("PRJEB22863_sample_number", "PRJNA751792_sample_number"),
                      labels = c("RoutyB_2018", "DerosaL_2022"),
                      values = mycolor7[c(1,2)])+
 theme(plot.subtitle = element_text(vjust =1), 
        plot.caption = element_text(vjust = 1), 
...
tiff(file = "pics/FS1_France_Samples_number_species.tiff", width =2600, height =1100, res =300) 
print(p_sample_n)
dev.off()
...
species_sample_n<-info[,c(1,10,15,11:14,17,18,19)]
...
tiff(file = "pics/FS1_France_Sample_n_SVs_n_scatter_pie.tiff", width =1200, height =800, res =300) 
print(p_scatter_pie)
dev.off()
...
tiff(file = "pics/FS1_France_sv_abun_density.tiff", width =2200, height =1600, res =300) 
print(p_sv_abun_density)
dev.off()
...
save(vsv_corr.edge, file = "04.SV_summary_statistics/France_vsv_corr.edge.RData")
...
save(dsv_corr.edge, file = "04.SV_summary_statistics/France_dsv_corr.edge.RData")
...
tiff(file = "pics/FS1_France_vsv_correlation.tiff", width =2000, height =2000, res =300) 
plot_grid(plotlist=all_vsv_M_plot.list)
dev.off()

tiff(file = "pics/FS1_France_dsv_correlation.tiff", width =2000, height =2000, res =300) 
plot_grid(plotlist=all_dsv_M_plot.list)
dev.off()
