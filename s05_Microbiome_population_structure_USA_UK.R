...
### 1.2 Inputs
load("01.cleanData/SV_all/USA_UK_all_shared_sv_dis.RData")
load("01.cleanData/SV_all/distMat/USA_UK_all_msv_dist_std.RData")
...
p_all_sv_pcoa<-ggplot(all_sv_avg_dist_pcoa,aes(X1,X2, color = all_basic_sv_rmna$dataset))+
  geom_point(size = 2,alpha = 0.8)+ggtitle("USA/UK")+
  ...
  scale_color_manual(name=NULL, 
                     breaks = c("PRJNA397906","PRJNA762360","PRJNA770295","PRJEB43119","PRJNA541981"),
                     labels = c("FrankelAE_2017","McCullochJA_2022","SpencerCN_2021","LeeKA_2022","PetersBA_2019"),
                     values = c("#0072B5FF","#20854EFF","#7876B1FF","#FFDC91FF","#E18727FF"))+
  guides(col = guide_legend(nrow = 2, byrow = TRUE))+ggtitle("USA/UK")+
...
tiff(file = "pics/F3C_USA_UK_all_SV_PCoA.tiff", width =3000, height =3000, res =600) 
print(p_all_sv_pcoa)
dev.off()
...
p_all_abun_pcoa<-ggplot(all_abun_dist_pcoa,aes(X1,X2, color = all_basic_abun_rmna$dataset))+
  ...
  xlab(paste("PCo1=",round(xVar,digits = 2),"%",sep = ""))+
  ylab(paste("PCo2=",round(yVar,digits = 2),"%",sep = ""))+ggtitle("USA/UK")+
  scale_color_manual(name=NULL, 
                     breaks = c("PRJNA397906","PRJNA762360","PRJNA770295","PRJEB43119","PRJNA541981"),
                     labels = c("FrankelAE_2017","McCullochJA_2022","SpencerCN_2021","LeeKA_2022","PetersBA_2019"),
                    values =c("#0072B5FF","#20854EFF","#7876B1FF","#FFDC91FF","#E18727FF"))+
...
tiff(file = "pics/FS2_USA_UK_all_abun_PCoA.tiff", width =3000, height =3000, res =600) 
print(p_all_abun_pcoa)
dev.off()
...
save(all_sv_dist.adonis, file = "05.Microbiome_population_structure/USA_UK_all_sv_dist.adonis.RData")
...
demo_factors<-c("gender_num","age_bins","X4","X5","X2","X3","X1","study")
...
all_basic_sv_p.adonis.comb<-adonis(as.dist(all_inter_sv_dist)~gender_num+age_bins+X4+X2+X3+X5+X1+study,data = all_inter_abun_pc_basic)
...
all_basic_sv_p_adonis.df$Factor[1:8]<-c( "Gender","Age","abun_PC4","abun_PC5","abun_PC2","abun_PC3","abun_PC1","Cohort")
write.table(all_basic_sv_p_adonis.df, "05.Microbiome_population_structure/all_basic_sv_p_adonis.tsv",
            sep = "\t", col.names = T, row.names = F, quote = F)

save(all_basic_sv_p_adonis.df, file = "05.Microbiome_population_structure/USA_UK_all_basic_sv_p_adonis.df.RData")


### 3.2 Visulization
demo_factors<-c("Gender","Age","abun_PC4","abun_PC5","abun_PC2","abun_PC3","abun_PC1","Cohort")

## ProPortion
load("05.Microbiome_population_structure/USA_UK_all_basic_sv_p_adonis.df.RData")
...
p_all_basic_sv_p_adonis<-ggplot(all_basic_sv_p_adonis.df.long, aes(Factor, Value,group = R2))+
  ...
  scale_fill_manual(name=NULL,
                      ...
                      values = mycolor2_green_blue)+ggtitle("USA/UK")+
  theme(plot.subtitle = element_text(vjust = 1), 
        plot.caption = element_text(vjust = 1), 
...
tiff(file = "pics/FS1_USA_UK_basic_sv_adonis_bar.tiff", width =2200, height =2200, res =300) 
print(p_all_basic_sv_p_adonis)
dev.off()
...
## all 
write.table(msv_pc_cum0.6, "01.cleanData/SV_all/USA_UK_all_msv_pc_cum0.6.tsv",sep = "\t", quote = F)
save(msv_pc_cum0.6, file = "01.cleanData/SV_all/USA_UK_msv_pc_cum0.6.RData")

## PC information
colnames(msv_pc_cum0.6_info)<-c("Species", "PC_number", "Explained_variance_proportion")
write.table(msv_pc_cum0.6_info, "01.cleanData/SV_all/USA_UK_all_msv_pc_cum0.6_info.tsv",col.names = T, row.names = F,sep = "\t", quote = F)
save(msv_pc_cum0.6_info, file = "01.cleanData/SV_all/USA_UK_msv_pc_cum0.6_info.RData")
